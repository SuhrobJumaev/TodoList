...
class Pagination
{
     ...
     * @return
     */
    public function __construct($total, $currentPage, $limit, $index,$order, $sort)
    {
    ...
    public function get()
    {
        ...
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            ...
            if ($page == $this->current_page) {
                $links .= '<li class="page-item active" class="active"><a  class="page-link"  href="#">' . $page .  '</a></li>';
            } else {
                # Иначе генерируем ссылку
                $links .= $this->generateHtml($page, null, $this->order, $this->sort);
            }
        }
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1, '&lt;',$this->order, $this->sort) . $links;
            # Если текущая страница не первая
            if ($this->current_page < $this->amount)
            # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, '&gt;',$this->order, $this->sort);
        }
    ...
    private function generateHtml($page, $text = null, $order, $sort)
    {
        ...
        $currentURI = preg_replace('~/page-[0-9]+/[a-zA-Z]+/[a-zA-Z]+~', '', $currentURI);
        ...
        return
               '<li class="page-item" ><a class="page-link" href="' . $currentURI . $this->index . $page  . '/' . $order . '/' . $sort . '">' .  $text . '</a></li>';
    }
